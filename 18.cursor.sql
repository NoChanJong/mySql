/*
	...
	2. Cursor의 종류
		 1) 묵시적(암묵적, implicit) 커서
				...
				대한 익명의 주소이다. 오라클데이터베이스에서 실행되는 모든 SQL문장은 묵시적 커서가 생성되면
				커서속성을 사용할 수 있다. 
				
				묵시적커서는 SQL문이 실행되는 순간 자동으로 open과 close가 된다. 암묵적커서의 속성은
				
				sql%rowcount 	: 해당 sql문에서 영향을 받는 행의 갯수
				sql%found 		: 해당 sql문에서 행의 갯수가 한개 이상일 때 true 리턴
				sql%not found : 해당 sql문에서 행의 갯수가 없을 때 true를 리턴
		 ...
		 2) 명시적(explicit) 커서		 
				...
				"cursor 선언 -> open -> fetch -> close"로 진행된다.
				
				a. 문법 : cursor 커서명 is select문(서브 쿼리)
				b. 사용
					 1) 커서열기(open)
							... 커서의 열기는 open문을 사용
					 ...
					 2) 커서패치(fetch)
							...
							... 커서컬럼의 데이터탕비과 output변수의 타입은 동일해야 한다.
							... 문법 : fetch 커서명 into 변수1...변수n
					 
					 3) 커서닫기(close)
							... 사용이 끝난 커서는 반드시 닫아주어야 한다.
...
-- 2) 여러건
-- for문을 사용하면 커서의 open, fetch, close가 자동으로 실행되기 때문에
...
select count(*) + 1 from sungresult where tot > 180; -- rank
...
select add_num(1, 2) 합계 from dual;
...
select get_emp_count(30) 사원수 from dual;


-- ex03) emp에서 사원번호를 입력하면 해당 사원의 관리자 이름을 구하는 함수
-- 함수명 get_mgr_name
select * from emp;
-- empno -> mgrno 가 맞는 관리자 이름 출력
...
select get_mgr_name(7698) from dual;
...
if v_sal between 4000 and 5000 
				 then v_result := 'A';
elsif v_sal between 3000 and 4000
						then v_result := 'B';
elsif v_sal between 2000 and 3000
						then v_result := 'C';
elsif v_sal between 1000 and 2000
						then v_result := 'D';
else v_result := 'F';
end if;
...
begin
	insert 
	into star_wars values(7, '새로운 공화국(New Republic)', 2009);
end;
