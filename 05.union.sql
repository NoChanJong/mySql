/*
	...
	2. union all : 두 집합의 결과를 합쳐서 출력 중복과 상관 없이 전체 자료 조회
	3. intersect : 두 집합의 교집합을 출력(정렬) 
	4. minus		 : 두 집합의 차집합을 출력(정렬), 선후가 중요

	[집합연산자의 조건]s
	1. 두 집합의 select절의 컬럼수가 동수이어야 한다.
	2. 두 집합의 select절의 같은 위치의 컬럼의 데이터타입(숫자, 문자,..)은 동일 데이터타입이어야 한다.
...
select studno 학생번호from student
union 
select profno 교수번호 from professor;

select name 교수명 from professor
union 
select studno 학생번호 from STUDENT; -- (x) 데이터타입이 다름

select studno 학생번호, name 학생명 from student
union 
select profno 교수번호 from professor; -- (x) 컬럼 갯수가 다름

-- 2. union/union all
select count(*) from STUDENT; -- 집계함수 count(*[컬럼명])
select count(deptno1) from STUDENT;
select count(deptno2) from STUDENT; -- null은 값이 아니여서 count안함.
