/*
	...
	2. 데이터타입
		 ...
		 4) lob타입 : LOB타입이란 Large Object의 약자로 대용량의 데이터를 저장할 수 있는 데이터타입
									일반적으로 그래프, 이미지, 동영상, 음악파일등 비정형 데이터를 저장할 떄 사용한다.
...
/* 테이블 생성시 주의사항
  ...
  1. 테이블이름은 반드시 문자로시작해야 한다. 중간에 숫자가 포함되는 것은 가능하다.
	2. 특수문자도 가능하지만 단, 테이블생성시 큰 따옴표로 감싸야한다. 하지만 권장하지는 않는다.
	3. 테이블이름이나 컬럼명은 최대 30byte까지 가능(즉, 한글은 문자셋에 따라 utf-8이면 10자, euc-kr이면 15자)
	4. 동일 사용자(스키마)안에서는 테이블명을 중복정의할 수 없다.
	5. 테이블명이나 컬럼명은 오라클키워드를 사용하지 않을 것을 권장(하지만 절대로 사용하지 말것)
*/

-- 1. 테이블에 자료를 추가하기(insert)
-- a. 문법
-- insert into mytable(컬럼1,....컬럼n) values(값1, ... 값n)
-- b. 제약사항
-- 1) 컬럼갯수와 값 갯수는 동수이어야 한다.
-- 2) 컬럼의 데이터타입과 값의 데이터타입은 동일해야 한다.
-- 		단, 형변환(자동형변환)이 가능하다면 사용할 수 있지만 변환이 불가능하다면 에러가 발생
select * from mytable;

insert into mytable(name) values('홍길동');
insert into mytable(name) values(1, '홍길동'); -- too many values Error
insert into mytable(no, name) values('홍길동'); -- not enough values Error
...
insert into mytable(no, name, hiredate) values('2023-03-27', '소향', '20230327'); -- invalid number

insert into mytable(no, name, hiredate) values(100, '거미', sysdate); -- value larger than specified precision allowed for this column
...
-- 2) 테이블 구조만 복사
-- 테이블이 이미 있는 경우 - name is already used by an existing object 에러
-- 테이블 삭제 명령
-- drop table 테이블명
...
/* B. 테이블 수정하기 - 컬럼
	...
	3. 데이터타입변경	 : alter table 테이블명 modify(변경할컬럼 변경할데이터타입)
...
-- 데이터타입을 변경할 경우 기존의 데이터에 따라 변경가능한 타입이라면 변경할 수 있지만
-- 변환불가능한 데이터가 있을 경우에는 에러 발생
alter table dept6 modify(dname number); -- error - column to be modified must be empty to change datatype
...
alter table dept7 modify(location varchar2(6)); -- (o)
...
/* 
  ...
  2. 데이터딕셔너리 뷰 : 오라클은 데이터딕셔너리의 내용을 사용자가 이해할 수 있는 내용으로 변환
		...
		2) all_xxx
			 ...
			 a. 자신계정소유와 권한을 부여 받은 객체등에 대한 정보를 조회할 수 있다.
...
-- http://www.gurubee.net -> oracle 강좌
